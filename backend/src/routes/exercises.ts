...
exercisesRouter.get('/', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
...
exercisesRouter.post('/', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const existing = await db.query.exerciseDefinitions.findFirst({
      where: and(
        eq(exerciseDefinitions.name, name.trim()),
        or(
          isNull(exerciseDefinitions.createdBy),
          eq(exerciseDefinitions.createdBy, userId)
        )
...
exercisesRouter.get('/:id/last-performance', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const workingSets = performanceSets.filter(s => s.type === 'working' && s.status === 'completed');
    const representativeSet = workingSets.length > 0 ? workingSets[0] : performanceSets[0];
