...
// 1. GET /measurements
measurementsRouter.get('/', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
...
// 2. POST /measurements
measurementsRouter.post('/', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
...
measurementsRouter.delete('/:id', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (!deleted) {
      return res.status(404).json({ error: 'Measurement not found or unauthorized' });
    }

    return res.json({ message: 'Measurement deleted successfully', id: deleted.id });
  } catch (error) {
    next(error);
  }
});
